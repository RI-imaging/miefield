...
def ric_besselj(nu,x):
    ...
    J = np.sqrt(np.pi/2.*(np.ones((len(nu),1))*x))* a      #HHH transposed a is horizontal.... multiplication creates a matrix.... bad.
    return J
...
def ric_bessely(nu,x):
    ...
    Y = np.sqrt(np.pi/2.*(np.ones((len(nu),1))*x)) * a       #HHH transpose is causing issues
...
def ric_besselj_derivative(nu, x, flag=1):
    ...
    if (flag == 1):
        J = 0.5*(  ric_besselj(nu-1, x)
                 + ric_besselj(nu,   x)* 1/temp
                 - ric_besselj(nu+1, x)           )
    elif (flag ==2):
        J = 0.5*(  ric_besselj_derivative(nu-1, x)
                 + ric_besselj_derivative(nu,   x)*  1/temp    
                 - ric_besselj(nu, x             )*  temp**(-2)
...
def kzlegendre(n,m,x):
    ...
    x = x.reshape(1, -1)
    if (np.size(n) > 1 and np.size(m) > 1):                 #HHH len() doens't work on integers...
    ...
    if (np.size(n) == 1 and np.size(m) >= 1):               #HHH len() doens't work on integers...
        if n > np.max(m):
            ...
            P = np.zeros((np.size(m), np.size(x)))          #HHH len() doens't work on integers...
            a = legendre(n,x)
            idx = np.where(m <= n)
            if np.sum(idx) != 0:
                #P = a(m(idx)+1,:);
        ...
        if m > np.max(m):
            P = np.zeros((np.size(n), np.size(x)))          #HHH len() doens't work on integers...
        else:
            P = np.zeros((np.size(n), np.size(x)))          #HHH len() doens't work on integers...
            for i in range(np.size(n)):                     #HHH len() doens't work on integers...
